...
namespace _Project.Code.Gameplay.GridFeature.Services
{
    public class ItemInventory
    {
        ...
        public void Move(CellGridPosition position, ItemPresenter item, bool silent = false) {
            _cells[position] = item;

            if (_shelves[position.Shelf].ColumnsCount <= position.Column) return;

            if (item != null) {
                _shelves[position.Shelf].MoveItem(item.Transform, position.Column, position.Layer);
